...
export const crearIntento = async (req: Request, res: Response) => {
  try {
    ...
    if (examen.intento_max && examen.intento_max > 0) {
      const count = await IntentoExamen.count({
...
export const registrarRespuestas = async (req: Request, res: Response) => {
  try {
    ...
    await RespuestaIntento.destroy({
      where: {
        intento_id: intento.id,
        pregunta_id: { [Op.in]: preguntaIds },
      },
    });
...
export const finalizarIntento = async (req: Request, res: Response) => {
  try {
    ...
    const respuestas = await RespuestaIntento.findAll({
    ...
    for (const p of (examen as any).preguntas as Pregunta[]) {
      ...
      const altCorrecta = (p as any).alternativas.find(
        (a: any) => a.es_correcta,
      );
      if (altCorrecta) {
    ...
    return res.json({
      ...
      porcentaje:
        puntajeTotalPosible > 0
          ? Number(
              ((puntajeObtenido / puntajeTotalPosible) * 100).toFixed(
                2,
              ),
            )
          : 0,
...
export const descargarPDFResultado = async (req: Request, res: Response) => {
  try {
    ...
    doc.text(
      "Este documento sirve como constancia del resultado del intento de examen.",
      { align: "left" },
    );
